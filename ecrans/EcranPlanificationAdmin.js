...
export default function EcranPlanificationAdmin({ route }) {
  ...
  useEffect(() => {
    ...
    tachesAujourdhui.forEach(tache => {
      showLocalNotification('📅 Rappel tâche (Admin)', `Aujourd'hui : ${tache.description}`);
    });
  }, [taches]);
  ...
  const ajouterTache = async () => {
    ...
    try {
      ...
      await showLocalNotification('✅ Tâche ajoutée (Admin)', `${nouvelleTache} prévue le ${dateTache}`);

      const [jour, mois, annee] = dateTache.split('/');
      const dateObj = new Date(`${annee}-${mois}-${jour}T08:00:00`);
      if (dateObj > new Date()) {
        await scheduleNotification('📌 Tâche planifiée (Admin)', nouvelleTache, dateObj);
      }
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>Planification (Admin)</Text>
